...
def detector_to_ray(point: torch.Tensor, poni: torch.Tensor) -> torch.Tensor:
    ...
    ----------
    point : torch.Tensor
        The 2d point in the referencial of the detector of shape (*p, *r, 2).
    poni : torch.Tensor
    ...
    -------
    ray : torch.Tensor
        The unitary rays vectors of shape (*broadcast(p, p'), *r, 3).
    """
    ...
    *batch_poni, _ = poni.shape
    batch_poni = torch.broadcast_shapes(batch_poni, batch[:len(batch_poni)])
    batch_ray = batch[len(batch_poni):]

    point = point - poni[..., *((None,) * len(batch_ray)), 1:3]  # detector shift
    ray = torch.empty(*batch_poni, *batch_ray, 3, dtype=point.dtype, device=point.device)
    ray[..., :2] = point
