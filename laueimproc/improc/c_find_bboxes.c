...
int FindClustersAndBBoxes(
    ...
    unsigned long* merge;
    unsigned long curr_clus = 0;
    unsigned long i, j, clus_top, clus_left, stride, shift_img;
    const unsigned long area = height * width;
    ...
    *bboxes_p = malloc(4 * (area + height + width) / 2 * sizeof(**bboxes_p));  // bigger than necessary
    ...
    for (shift_img = 0; shift_img < area; shift_img += width) {
        for (j = 0, clus_left = 0; j < width; clus_left = clusters[j++]) {
            if (!binary[shift_img + j]) {  // case black pixel
                clusters[j] = 0;
                continue;
            }
            clus_top = clusters[j];
