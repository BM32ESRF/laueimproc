...
def compute_barycenters(tensor_spots: Tensor) -> Tensor:
    ...
    tensor_spots : laueimproc.classes.tensor.Tensor
        The batch of spots of shape (n, w, h).
    ...
    * Call this method from a ``laueimproc.classes.spot.Spot``
    ...
    widths, heights = (  # optim: height_ind column invariant
        torch.sum(tensor_spots, axis=2, keepdim=True), torch.sum(tensor_spots, axis=1, keepdim=True)
    )
    widths *= width_ind
    heights *= height_ind
    widths, heights = torch.sum(widths, axis=2), torch.sum(heights, axis=1)
    pos = torch.cat((widths, heights), 1)
